...
CREATE TABLE user_invitations (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK ((status = 'pending' AND claimed_by IS NULL AND claimed_at IS NULL AND cancelled_by IS NULL AND cancelled_at IS NULL)
...
CREATE UNIQUE INDEX idx_user_invitations_open_email
...
CREATE TABLE trainers (
  ...
  name                           TEXT        NOT NULL,
  is_active                      BOOLEAN     NOT NULL DEFAULT TRUE,
  module_excludes                TEXT[]      DEFAULT '{}',  -- rollback-compatible projection; trainer_course_exclusions is authoritative
  notes                          TEXT,
  version                        INTEGER     NOT NULL DEFAULT 1,
  scheduling_readiness           scheduling_readiness NOT NULL DEFAULT 'needs_setup',
  readiness_origin               readiness_origin,
  readiness_confirmed_at         TIMESTAMPTZ,
  readiness_confirmed_by         UUID        REFERENCES users (id),
  readiness_version              INTEGER,
  eligibility_version            INTEGER     NOT NULL DEFAULT 1,
  exclusions_acknowledged_at     TIMESTAMPTZ,
  exclusions_acknowledged_by     UUID        REFERENCES users (id),
  exclusions_acknowledged_version INTEGER,
  created_at                     TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at                     TIMESTAMPTZ NOT NULL DEFAULT now(),

  CONSTRAINT chk_trainers_version_positive CHECK (version > 0),
  CONSTRAINT chk_trainers_eligibility_version_positive CHECK (eligibility_version > 0),
  CONSTRAINT chk_trainers_readiness_version_positive CHECK (
    readiness_version IS NULL OR readiness_version > 0
  ),
  CONSTRAINT chk_trainers_readiness_version_current CHECK (
    readiness_version IS NULL OR readiness_version <= version
  ),
  CONSTRAINT chk_trainers_readiness_state CHECK (
    (scheduling_readiness = 'ready' AND readiness_origin IS NOT NULL AND readiness_version IS NOT NULL)
  ...
  CONSTRAINT chk_trainers_readiness_origin CHECK (
  ...
  CONSTRAINT chk_trainers_exclusions_acknowledgement CHECK (
  ...
  CONSTRAINT chk_trainers_notes_length CHECK (
...
CREATE UNIQUE INDEX idx_trainer_aliases_alias_name_normalized
...
-- ---------------------------------------------------------------------------
-- 4a. Trainer directory migration state
--     Durable one-time markers keep upgrade-only initialization from treating
--     later version-1 trainers as pre-existing records.
-- ---------------------------------------------------------------------------
CREATE TABLE trainer_directory_migration_state (
  migration_key TEXT PRIMARY KEY,
  applied_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- ---------------------------------------------------------------------------
-- 4b. Trainer course exclusions
--     This normalized table is authoritative. `trainers.module_excludes` is
--     retained as a rollback-compatible projection for older readers. Future
--     eligibility writes must update both representations in one transaction;
--     no database trigger is installed because legacy import writes still
--     target the projection and a bidirectional trigger would be unsafe.
-- ---------------------------------------------------------------------------
CREATE TABLE trainer_course_exclusions (
  trainer_id  TEXT NOT NULL REFERENCES trainers (trainer_id) ON UPDATE CASCADE,
  course_code TEXT NOT NULL REFERENCES courses (code) ON UPDATE CASCADE,
  PRIMARY KEY (trainer_id, course_code)
);

CREATE INDEX idx_trainer_course_exclusions_course
  ON trainer_course_exclusions (course_code);

-- ---------------------------------------------------------------------------
-- 4c. Shared reference-data finalization
--     The migration and fresh-bootstrap seed invoke the same function. The
--     marker insert is the one-time claim; all state changes are in this
--     function's transaction and a failed call rolls back the claim.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4d. Trainer change history
-- ---------------------------------------------------------------------------
CREATE TABLE trainer_change_events (
  id                           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_trainer_change_events_trainer_time
  ON trainer_change_events (trainer_id, created_at DESC);
CREATE INDEX idx_trainer_change_events_actor
...
-- Keep the normalized authority in step with the legacy seed projection.
INSERT INTO trainer_course_exclusions (trainer_id, course_code)
SELECT t.trainer_id, btrim(exclusion_code)
FROM trainers AS t
CROSS JOIN LATERAL unnest(COALESCE(t.module_excludes, ARRAY[]::TEXT[]))
  AS exclusions(exclusion_code)
WHERE btrim(exclusions.exclusion_code) <> ''
