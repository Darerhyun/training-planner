...
Claude loads this brief, then opens a Claude Design canvas. Approved mockups are exported as PNG and committed to the repository together with this brief (proposed `docs/03-design/`) so they are immutable review inputs before PR3G-V is authorised; Luna Max owns delivery, delegates bounded difficult execution to Astra Low, and integrates and validates changes in `styles.css` / `App.tsx` with no API, auth, backend, dependency, or infra changes. Sol High independently reviews technical correctness after implementation and cannot implement the same work item. Claude reviews UI/IX conformance against the same exact head; neither reviewer edits. Claude's recommendations pass Sol's architecture gate; Claude is not mandatory for non-UI work unless Owen requests it.
