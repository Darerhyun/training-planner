...
export default function AdminTrainerDirectoryPage({ user, onApiError, onDirtyChange }: { user: User; onApiError: (error: unknown, setError: (message: string) => void) => Promise<void>; onDirtyChange: (dirty: boolean) => void }) {
  ...
  async function checkImpact() {
    ...
    try { const result = await fetchTrainerImpact(user, detail.trainer_id); if (result.version !== detail.version) { setStale(true); setImpactError('This trainer changed. Reload trainer before deactivating.'); } else setImpact(result.upcomingSessionCount); }
    catch (failure) { if (failure instanceof ApiError && failure.status === 404) await unavailable(); else await reportFailure(failure, setImpactError); }
  }
  async function save(input: TrainerMutation | null, success: string) {
    ...
    try {
      ...
      if (trainerAccessDenied(failure)) await reportFailure(failure, setError);
        ...
        if (failure instanceof ApiError && failure.code?.startsWith('stale_trainer')) { setStale(true); setDialog(null); focusLater(dialogOpener.current); }
        await reportFailure(failure, setError);
        if (input?.action === 'remove-alias') { setDialog(null); focusLater(dialogOpener.current); }
        else requestAnimationFrame(() => document.querySelector<HTMLElement>('.trainer-modal input, .trainer-modal textarea, .trainer-drawer input')?.focus());
      }
  ...
  return <section className="trainer-directory" aria-label="Trainer Directory">
    ...
    {listError ? <div role="alert"><p>{listError.includes('403') || listError.toLowerCase().includes('authorized') ? 'Admin access is required for Trainer Directory.' : 'Trainer records could not be loaded.'}</p><button className="secondary" onClick={() => void loadList()}>Retry</button></div> : <div aria-busy={listLoading}>
      ...
      {!!list?.trainers.length && <><table className="trainer-table"><colgroup>{[29, 16, 14, 10, 13, 18].map((width) => <col key={width} style={{ width: `${width}%` }} />)}</colgroup><thead><tr>{['Trainer', 'Readiness', 'Eligible courses', 'Exclusions', 'Upcoming sessions', 'Last updated'].map((title) => <th key={title}>{title}</th>)}</tr></thead><tbody>{list.trainers.map((row) => <tr key={row.trainer_id} tabIndex={0} aria-selected={selectedId === row.trainer_id} onClick={(event) => openTrainer(row.trainer_id, event.currentTarget)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openTrainer(row.trainer_id, event.currentTarget); } }}><td><strong>{row.name}</strong><small title={row.aliases.map((a) => a.alias_name).join(' · ')}>{row.aliases.map((a) => a.alias_name).join(' · ')}</small>{search && row.matched_course && <small>Matched course: {row.matched_course}</small>}</td><td><span className={`trainer-pill ${stateOf(row)}`}>{labels[stateOf(row)]}</span>{state === 'needs_setup' && <small>{row.eligible_course_count ? 'Review exclusions and confirm' : 'Add eligible courses'}</small>}</td><td aria-label={`${row.eligible_course_count} eligible courses, ${row.sme_count} subject matter expert links`}>{row.eligible_course_count} eligible · {row.sme_count} SME</td><td>{row.exclusion_count || 'None'}</td><td>{row.upcoming_session_count}</td><td><small>{dateLabel(row.updated_at)}</small><small>{row.updated_by?.displayName || row.updated_by?.email || 'Not recorded'}</small></td></tr>)}</tbody></table>
      <div className="trainer-cards">{list.trainers.map((row) => <article key={row.trainer_id} className={selectedId === row.trainer_id ? 'selected' : ''}><div className="trainer-heading"><h3>{row.name}</h3><span className={`trainer-pill ${stateOf(row)}`}>{labels[stateOf(row)]}</span></div><p>{row.aliases.map((a) => a.alias_name).join(' · ')}</p>{search && row.matched_course && <p>Matched course: {row.matched_course}</p>}{state === 'needs_setup' && <p>{row.eligible_course_count ? 'Review exclusions and confirm' : 'Add eligible courses'}</p>}<p>{row.eligible_course_count} eligible · {row.sme_count} SME · {row.exclusion_count} exclusions</p><p>{row.upcoming_session_count} upcoming sessions</p><p>{dateLabel(row.updated_at)} · {row.updated_by?.displayName || row.updated_by?.email || 'Not recorded'}</p><button className="secondary" onClick={(event) => openTrainer(row.trainer_id, event.currentTarget)}>View trainer<span className="sr-only"> {row.name}</span></button></article>)}</div></>}
    ...
    {selectedId && <div className="trainer-drawer-backdrop"><aside ref={drawer} className="trainer-drawer" role="dialog" aria-modal="true" aria-label="Trainer details" onKeyDown={(event) => { if (!dialog) trap(event, drawer.current, closeDrawer); }}>
      ...
      {stale && <div role="alert"><p>This trainer changed after you opened it. Reload the latest version before saving.</p><button className="secondary" onClick={() => { if (!dirty || window.confirm('Discard unsaved input and reload trainer?')) void loadDetail(selectedId); }}>Reload trainer</button></div>}
